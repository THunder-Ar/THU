...
ZelzalWF_cmd = (
    "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗛𝘂𝗻𝗱𝗲𝗿 - **اوامـر الـردود/الترحيب 🎡** 𓆪\n"
    "**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**\n\n"
    ...
    "\n 𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗛𝘂𝗻𝗱𝗲𝗿](t.me/SOURCE_THUNDER) 𓆪"
)
...
async def on_snip_list(event):
    ...
    for filt in filters:
        if OUT_STR == "** ❈╎لاتوجـد ردود محفوظـه في هـذه الدردشـه ༗**":
            OUT_STR = "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗛𝘂𝗻𝗱𝗲𝗿 - قائمـة الـردود 𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "👉 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def _(event):
    ...
    if gvarstatus("TIME_STOP") is not None: #Code by T.me/DAD_E3DAM
