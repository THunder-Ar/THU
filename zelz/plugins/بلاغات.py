...
#
""" 
Abuse Notifications for 𝚃𝚑𝚞𝚗𝚍𝚎𝚛™ t.me/SOURCE_THUNDER
...
async def spam_abusezed(event, sandy, zed, sleeptimem, sleeptimet, DelaySpam=False):
    ...
    if len(zed) == 2:
        ...
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message('@AbuseNotifications', spam_message) # code by t.me/DAD_E3DAM
            await asyncio.sleep(4)
    else:
        return
    if DelaySpam is not True: # code by t.me/DAD_E3DAM
        if BOTLOG:
