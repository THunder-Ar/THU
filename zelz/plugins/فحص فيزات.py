...
""" 
CC Checker & Generator for 𝗧𝗛𝘂𝗻𝗱𝗲𝗿™ t.me/SOURCE_THUNDER
Write file by 𝗧𝗛𝘂𝗻𝗱𝗲𝗿 t.me/SOURCE_THUNDER
hhh o ya beby
...
# code by t.me/SOURCE_THUNDER
@zedub.zed_cmd(pattern="توليد(?:\s|$)([\s\S]*)")
async def song2(event):
    been = event.pattern_match.group(1)
    chat = "@SDBB_Bot" # code by t.me/SOURCE_THUNDER
    reply_id_ = await reply_id(event)
    zed = await edit_or_reply(event, f"**⎉╎جـارِ جلب كومبـو لـ البين {been}  ...**\n**⎉╎عـدد 10 بطاقـات 💳**")
    async with event.client.conversation(chat) as conv:
        try:
...
async def song2(event):
    been = "524447000053xxxx|xx|xxxx|xxx" # code by t.me/SOURCE_THUNDER
    chat = "@SDBB_Bot" # code by t.me/SOURCE_THUNDER
    reply_id_ = await reply_id(event)
...
async def song2(event):
    been = "373352589xxxxxx|xx|xxxx|xxxx" # code by t.me/SOURCE_THUNDER
    chat = "@SDBB_Bot" # code by t.me/SOURCE_THUNDER
    reply_id_ = await reply_id(event)
