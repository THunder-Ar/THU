...
حقـوق للتـاريخ : @SOURCE_THUNDER
@DAD_E3DAM - كتـابـة الملـف : المطور اعدام  """
#المطور_اعدام يولـد هههههههههههههههههههههههههه
import asyncio
import math
import os
...
ZelzalVP_cmd = (
    "𓆩 [𝗧𝗛𝘂𝗻𝗱𝗲𝗿 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/SOURCE_THUNDER) 𓆪\n\n"
    ...
    "⪼ `.اضف كليشة الايدي` بالـرد ع الكليشـه مـن القنـاة @THUNDERV1 \n\n\n"
    ...
    "⪼ قنـاة كلايـش حمايـة الخـاص @THUNDERV1\n\n"
    ...
    "\n𓆩 [𝗧𝗛𝘂𝗻𝗱𝗲𝗿 𝗩𝗮𝗿𝘀 - قنـاة الفـارات](t.me/THUNDERV1) 𓆪"
)
...
async def variable(event):
    ...
    # All Rights Reserved for "𝗧𝗛𝘂𝗻𝗱𝗲𝗿" "المطور اعدام"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ثانــدر يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم اضافـة {} بنجـاح ☑️** \n**⎉╎المضاف اليه :**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ثانــدر يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ثانــدر يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, variable))
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ثانــدر يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        if variable is None:
            ...
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @SOURCE_THUNDER**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("**⎉╎فـار ايموجي/رمز الايدي غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع الكليشـة من هنا ( @THUNDERV1 ) استخـدم الامـر : ↶**\n `.اضف كليشة الايدي` \n\n**⎉╎قنـاة السـورس : @SOURCE_THUNDER**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @SOURCE_THUNDER**".format(input_str, variable))
...
async def variable(event):
    ...
    viran = "Asia/Tehran"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**⎉╎جـاري أعـداد المنطقـه الزمنيـه لـ ثانــدر 🌐...**")
    # All Rights Reserved for "𝗧𝗛𝘂𝗻𝗱𝗲𝗿" "المطور اعـدام"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ثانــدر يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
