...
async def ge(user, event):
    ...
    try:
        ...
        return None
    return user_obj
########################  ZThon Userbot ~ By: E3DAM (@DAD_E3DAM)  ########################
@zedub.zed_cmd(pattern="رابط الحذف")
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗧𝗛𝘂𝗻𝗱𝗲𝗿 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/SOURCE_THUNDER) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗧𝗛𝘂𝗻𝗱𝗲𝗿─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف¹  ↬** @LC6BOT\n**✵│بـوت الحـذف²  ↬** @DTeLebot")
########################  ZThon Userbot ~ By: E3DAM (@DAD_E3DAM)  ########################
...
########################  ZThon Userbot ~ By: E3DAM (@DAD_E3DAM)  ########################
@zedub.zed_cmd(pattern="رفع مراتي(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
########################  ZThon Userbot ~ By: E3DAM (@DAD_E3DAM)  ########################
@zedub.zed_cmd(pattern="رفع وتقه(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
########################  ZThon Userbot ~ By: E3DAM (@DAD_E3DAM)  ########################
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗛𝘂𝗻𝗱𝗲𝗿 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/SOURCE_THUNDER) 𓆪\n\n"
...
async def permalink(malath):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(malath, f"**╮ ❐ لك هـذا المطور اعدام . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
